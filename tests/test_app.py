...
def test_index_route(client):
    ...
    assert b"W\xc3\xb6rtWeaver" in response.data or b"WortWeaver" in response.data or b"Spot-Check" in response.data
...
def test_upload_file_txt(client):
    data = {
        'file': (io.BytesIO(b"Guten Tag Welt!\n\nDies ist eine Testdatei."), "sample.txt")
    }
...
def test_upload_file_invalid_extension(client):
    data = {
        'file': (io.BytesIO(b"binary data"), "image.png")
    }
    response = client.post("/upload", data=data, content_type='multipart/form-data')
    assert response.status_code == 400
    json_data = response.get_json()
